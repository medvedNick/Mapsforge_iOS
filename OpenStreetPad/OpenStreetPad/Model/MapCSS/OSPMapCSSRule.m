...
{
    NSMutableArray *matchingLayerIdentifiers = nil;// = [[NSMutableArray alloc] init];
    
    for (OSPMapCSSSelector *selector in selectors)
    {
        if ([selector matchesObject:object atZoom:zoom])
        {
			if (matchingLayerIdentifiers == nil) matchingLayerIdentifiers = [NSMutableArray array];
            [matchingLayerIdentifiers addObject:[selector layerIdentifier]];
        }
    }
    
    if (matchingLayerIdentifiers != nil)
    {
...
//						[newList release];
    ...
    return nil;//[NSDictionary dictionary];
}
